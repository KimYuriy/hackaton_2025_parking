...
import 'package:parking_admin/components/pages/places/ui/common/places_table/add_parking_place_alert.dart';
          ...
          children: [
            Flexible(
              ...
              child: SingleChildScrollView(
                child: DataTable(
                  columns: [
                    const DataColumn(label: Text('№ места')),
                    const DataColumn(label: Text('Тип места')),
                    const DataColumn(label: Text('ФИО владельца')),
                    const DataColumn(label: Text('№ машины')),
                    const DataColumn(label: Text('Статус')),
                    DataColumn(label: TextButton(
                      onPressed: () => showDialog(
                        context: context,
                        builder: (context) => AddParkingPlaceAlert(onComplete: (json) {
                          context.read<PlacesTableBloc>().add(PlacesTableEvents.addPlace(json: json));
                        }),
                      ),
